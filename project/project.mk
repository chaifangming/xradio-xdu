...
LIBRARIES := -Wl,--whole-archive -lchip -lrom -lboard -Wl,--no-whole-archive
LIBRARIES += -lcmd -lframework -liperf -lstartup
...
.PHONY: all $(PROJECT).$(ELF_EXT) objdump size clean lib lib_clean \
	...
	config config_clean common common_clean

all: $(PROJECT).bin size

common:
	$(Q)$(MAKE) $(S) -C $(ROOT_PATH)/project/common install PRJ_BOARD=$(abspath $(PRJ_BOARD)) PRJ_ROOT_PATH=$(abspath $(PRJ_ROOT_PATH))
...
common_clean:
	$(Q)$(MAKE) $(S) -C $(ROOT_PATH)/project/common clean
...
build: common lib all install

build_clean: clean lib_clean lib_install_clean common_clean
...
build: common lib all image

build_clean: image_clean clean lib_clean lib_install_clean common_clean
